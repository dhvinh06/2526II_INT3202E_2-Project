...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      } catch (err) { setError(err.message); setProducts(MOCK_PRODUCTS) }
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const map = {}
        children.forEach(c => { if (!map[c.parentId]) map[c.parentId] = []; map[c.parentId].push(c) })
        setParentCategories(parents); setChildrenMap(map)
    ...
    <div className={styles.page}>
      ...
      <section className={styles.tileLight}>
        <div className={styles.tileInner}>
          ...
          <p className={`t-lead ${styles.heroLead}`}>Hàng ngàn sản phẩm chất lượng, giao hàng nhanh toàn quốc.</p>
      ...
      <section className={styles.tileLight}>
        <div className={styles.containerWide}>
          ...
          {!isLoading && <div className={styles.grid}>{recommendedProducts.map(p => <ProductCard key={p.id} product={p} />)}</div>}
        </div>
      </section>
      ...
      <section className={styles.tileLight}>
        <div className={styles.containerWide}>
          ...
          {!isLoading && <div className={styles.grid}>{bestSellers.map(p => <ProductCard key={p.id} product={p} />)}</div>}
        </div>
      </section>
