...
const MOCK_ORDERS = [
  { id:'ORD-982341', date:'10/05/2026', status:'DELIVERED', total:394000, items:[{ id:1, name:'Áo thun nam basic oversize Hàn Quốc', quantity:2, image:'https://picsum.photos/seed/pr1/80/80' }] },
  { id:'ORD-554123', date:'11/05/2026', status:'CONFIRMED', total:890000, items:[{ id:2, name:'Tai nghe Bluetooth ANC chống ồn', quantity:1, image:'https://picsum.photos/seed/pr2/80/80' }] },
  { id:'ORD-109283', date:'12/05/2026', status:'PENDING',   total:520000, items:[{ id:5, name:'Giày sneaker nam nữ đế êm thoáng khí', quantity:1, image:'https://picsum.photos/seed/pr5/80/80' }] }
]
const STATUS_MAP = {
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (res) { setProfileForm({ name: res.name, email: res.email }); login({ ...user, name: res.name, email: res.email }) }
      } catch (err) {}
    }
    fetchUser()
  ...
  const handleInfoSubmit = async (e) => {
    ...
    setInfoLoading(true)
    try {
      try { await userAPI.updateProfile(user.id, profileForm) } catch (apiErr) {}
  ...
  const handlePassSubmit = async (e) => {
    ...
    setPassLoading(true)
    try {
      try { await userAPI.changePassword(user.id, { oldPassword: passForm.oldPassword, newPassword: passForm.newPassword }) } catch (apiErr) {}
  ...
  const renderPasswordTab = () => (
    <div className={styles.tabPanel}>
      ...
      <form className={styles.form} onSubmit={handlePassSubmit}>
        <PasswordField label="Mật khẩu hiện tại" field="oldPassword" show={showOld} setShow={setShowOld} />
        <PasswordField label="Mật khẩu mới" field="newPassword" show={showNew} setShow={setShowNew} />
        <PasswordField label="Xác nhận mật khẩu mới" field="confirmPassword" show={showConfirm} setShow={setShowConfirm} />
          ...
          <div key={order.id} className={styles.orderCard}>
            <div className={styles.orderHeader}>
              <div className={styles.orderMeta}>
                ...
                <span className={`t-caption ${styles.orderDate}`}>{order.date}</span>
              </div>
              <div className={`${styles.orderStatus} ${styles[STATUS_MAP[order.status].color]} t-caption-strong`}>{STATUS_MAP[order.status].label}</div>
            </div>
            ...
            <div className={styles.orderFooter}>
              <div className={`t-body ${styles.orderTotal}`}>Tổng tiền: <span className="t-body-strong">{fmt(order.total)}</span></div>
