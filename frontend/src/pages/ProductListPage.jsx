...
const CATEGORIES = [
  { id: 'all', name: 'Tất cả' },{ id: 'fashion', name: 'Thời trang' },{ id: 'electronics', name: 'Điện tử' },
  { id: 'home', name: 'Gia dụng' },{ id: 'beauty', name: 'Làm đẹp' },{ id: 'books', name: 'Sách' },{ id: 'accessories', name: 'Phụ kiện' }
]
const SORT_OPTIONS = [
...
export default function ProductListPage() {
  ...
  useEffect(() => {
    setFilters(prev => ({ ...prev, search: searchParams.get('search') || '', category: searchParams.get('category') || 'all', sortBy: searchParams.get('sort') || 'newest' }))
    setCurrentPage(1)
  }, [searchParams])
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true)
      try {
        const data = await productAPI.getProducts({ search: filters.search || undefined, categoryId: filters.category !== 'all' ? filters.category : undefined })
        setProducts(data && data.length > 0 ? data : MOCK_PRODUCTS)
      } catch (err) { setError('Không thể tải dữ liệu sản phẩm.'); setProducts(MOCK_PRODUCTS) }
  ...
  const handlePriceApply = (e) => { e.preventDefault(); const f = e.target; handleFilterChange('minPrice', f.min.value); handleFilterChange('maxPrice', f.max.value) }
  const resetFilters = () => { setFilters({ search:'', category:'all', minPrice:'', maxPrice:'', sortBy:'newest' }); setSearchParams(new URLSearchParams()); setCurrentPage(1) }
      ...
      <div className={styles.container}>
        ...
        <main className={styles.main}>
          ...
          {loading ? (
            ...
            <>
              <div className={styles.grid}>{paginatedProducts.map(p => <ProductCard key={p.id} product={p} />)}</div>
              {totalPages > 1 && (
